...
namespace DiGi.ComputeSharp.Spatial.Classes
{
    ...
    /// <para>The <c>rowOffset</c> field is intentionally declared first: it fills the 4 bytes reserved in the constant buffer's second row, so the buffer keeps the exact 48-byte layout of <see cref="Triangle3ExternalShadingComputeShader"/>. Appending the field instead would grow the buffer, and on some drivers the grown buffer combined with the second structured buffer resource causes every intersection result to be discarded.</para>
    /// </summary>
