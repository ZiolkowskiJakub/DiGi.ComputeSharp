...
The `rowOffset` field is intentionally declared first: ComputeSharp places it at byte offset 12 of the constant buffer, in the 4 padding bytes after the `__x`/`__y`/`__z` dispatch header, so the buffer keeps the exact 56-byte layout of [Triangle3ExternalShadingComputeShader](DiGi.ComputeSharp.Spatial.Classes.md#DiGi.ComputeSharp.Spatial.Classes.Triangle3ExternalShadingComputeShader 'DiGi\.ComputeSharp\.Spatial\.Classes\.Triangle3ExternalShadingComputeShader') (`tolerance` at offset 16, `vector` at offsets 32 to 55). Appending the field instead would place it at offset 56 and grow the buffer to 60 bytes, and on some drivers the grown buffer combined with the second structured buffer resource causes every intersection result to be discarded (ZiolkowskiJakub/DiGi.ComputeSharp#3).
